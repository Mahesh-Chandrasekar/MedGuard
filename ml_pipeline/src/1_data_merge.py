...
# --- NEW: Lightweight Clinical Knowledge Base ---
DRUG_CLASSES = {
    ...
    "glycopyrronium": "Anticholinergic (COPD Treatment)",
    "tiotropium": "Anticholinergic (COPD Treatment)"
}
# ----------------------------------------------
...
def check_alternative_safety(drug_a, alt_drug, model):
    """Silently tests an alternative drug against Drug A"""
    fp_a = fetch_chemistry(drug_a)
    fp_alt = fetch_chemistry(alt_drug)
    cols_A = [f"A_chem_{i}" for i in range(256)]
    cols_B = [f"B_chem_{i}" for i in range(256)]
    input_data = pd.DataFrame([fp_a + fp_alt], columns=cols_A + cols_B)
...
def explainable_clinical_check(drug_a, drug_b):
    ...
    if severity == 'Major':
        ...
        print("   Action                 : High risk. See alternative therapy below.")
    elif severity == 'Moderate':
        print(f"⚠️ WARNING                : {severity.upper()} INTERACTION DETECTED")
        print(f"   Model Confidence       : {confidence:.1f}%")
        print("   Action                 : Monitor closely, or consider alternative therapy below.")
    else:
    ...
    if severity in ['Major', 'Moderate']:
        target_class = DRUG_CLASSES.get(drug_b.lower())
        
        if target_class:
            print("\n" + "="*75)
            print("💡 CLINICAL DECISION SUPPORT (Alternative Therapy):")
            print("="*75)
            print(f"   Drug B ({drug_b.title()}) is classified as: {target_class}")
            print(f"   Searching for safer alternatives to pair with {drug_a.title()}...\n")
            
            # Find other drugs in the same class
            alternatives = [d for d, c in DRUG_CLASSES.items() if c == target_class and d != drug_b.lower()]
            found_safe = False
            
            for alt in alternatives:
                print(f"   🧪 Testing alternative: [ {alt.upper()} ]...")
                alt_prediction_encoded = check_alternative_safety(drug_a, alt, model)
                alt_severity = encoder.inverse_transform([alt_prediction_encoded])[0]
                
                if alt_severity == 'Minor':  # 'Minor' is our 'Safe' label
                    print(f"   ✅ SUCCESS: {alt.title()} has no structural conflicts with {drug_a.title()}.")
                    print(f"   ▶️ RECOMMENDATION: Switch {drug_b.title()} to {alt.title()}.")
                    found_safe = True
                    break
                else:
                    print(f"   ❌ FAILED: {alt.title()} also triggers a {alt_severity} interaction.")
            
            if not found_safe:
                print(f"   ⚠️ Could not find a mathematically safe alternative in the current dictionary.")
    print("="*75 + "\n")
